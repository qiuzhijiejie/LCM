...
void OLED_Init(void)
{
    ...
    OLED_WR_Byte(0x20, OLED_CMD); // Set Memory Addressing Mode = Page addressing

    OLED_WR_Byte(0xA0, OLED_CMD); // Set Segment Re-map (0xA0/0xA1)
    OLED_WR_Byte(0xC0, OLED_CMD); // Set COM Output Scan Direction (0xC0/0xC8)

    OLED_WR_Byte(0xA6, OLED_CMD); // Normal Display (0xA6) / Inverse (0xA7)

    OLED_WR_Byte(0xA8, OLED_CMD); // Set Multiplex Ratio
    OLED_WR_Byte(0x7F, OLED_CMD); // Duty = 1/128

    OLED_WR_Byte(0xD3, OLED_CMD); // Set Display Offset
    OLED_WR_Byte(0x60, OLED_CMD); // 常见模组要加偏移 0x60

    OLED_WR_Byte(0xAD, OLED_CMD); // Set DC-DC Control Mode
    OLED_WR_Byte(0x8A, OLED_CMD); // DC-DC ON
...
{
    for ( row = 0; row < 16; row++) { // OLED 128x64 的 8 页（每页 8 行）
...
void OLED_Fill_Column(int oddData, int evenData) 
{
    ...
    for (page = 0; page < 16; page++) { // OLED 128x64 屏幕有 8 页
        OLED_Set_Pos(0, page); // 设置当前页的位置
        for (column = 0; column < 128; column++) { // 每页有 128 列
            if (column % 2 == 0) { // 偶数列
