...
void display_digit(unsigned char start_addr, SegmentData seg, unsigned char count)
{
    unsigned char addr = start_addr;
    for (i = 0; i < count; i++) 
	{
        write_data(addr, seg.high);     // 高4位
        write_data(addr + 2, seg.low);  // 低4位
        addr += 4;
    }
}
...
//        delay(100);
//        display_digit(0x41, digits[i], 11);  // 第二行（错位）
//        delay(3000);
//    }
//}
