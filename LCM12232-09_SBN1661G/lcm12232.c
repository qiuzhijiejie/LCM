...
{	
	unsigned char start_page = 0;  // 起始页
	unsigned char start_col = 16; // 起始列
	unsigned char spacing = 16;   // 每个字符之间的间距
	unsigned char chars_per_page = 5; // 每页显示的字符数量
	unsigned char total_pages = 2;    // 总页数
    unsigned char page,i;
	unsigned char character_code = 0; // 起始字符编码
	...
	for ( page = 0; page < total_pages; page++) 
	{
		...
		{
			unsigned char col = start_col + i * spacing; // 计算列位置
			Display_One_Chinese_16x16(start_page + page * 2, col, character_code, ft);
			character_code++; // 每次显示后编码递增
		}
	}
...
void Display_One_Emglish(void)
{
	...
	unsigned char i;
	unsigned char start_col = 3; // 起始列
	unsigned char char_count = 16; // 显示的字符数量
	unsigned char spacing = 7; // 每个字符之间的间距
	unsigned char page = 1; // 起始页
...
{
	unsigned char i;
	unsigned char start_col = 15;    // 起始列号
	unsigned char char_count = 11; // 显示的字符数量
	unsigned char spacing = 8;     // 每个字符之间的列间距
	unsigned char start_code = 16; // 起始字符编码
	unsigned char page = 1;        // 起始页
