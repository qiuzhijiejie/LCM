 unsigned char code guang[] =          // 数据表
{
/*-- ID:0,字符:"深",ASCII编码:C9EE,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x10,0x22,0x64,0x0c,0x88,0x06,0x22,0x12,0x0a,0xe2,0x0a,0x12,0x22,0x8a,0x06,0x00,
0x04,0x04,0xfe,0x01,0x10,0x11,0x09,0x05,0x03,0xff,0x03,0x05,0x09,0x19,0x09,0x00,
};

 unsigned char code don[] =          // 数据表
{
/*-- ID:1,字符:"圳",ASCII编码:DBDA,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x20,0x20,0x20,0xff,0x20,0x30,0x20,0xfe,0x00,0x00,0xfc,0x00,0x00,0xff,0x00,0x00,
0x08,0x18,0x08,0x0f,0x04,0x44,0x30,0x0f,0x00,0x00,0x1f,0x00,0x00,0xff,0x00,0x00,
};

 unsigned char code ye[] =          // 数据表
{
/*-- ID:2,字符:"市",ASCII编码:CAD0,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x08,0x08,0xc8,0x48,0x48,0x48,0x49,0xfa,0x48,0x48,0x48,0x48,0xe8,0x4c,0x08,0x00,
0x00,0x00,0x3f,0x00,0x00,0x00,0x00,0xff,0x00,0x00,0x10,0x20,0x1f,0x00,0x00,0x00,
};

 unsigned char code hui[] =          // 数据表
{
/*-- ID:3,字符:"众",ASCII编码:D6DA,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x00,0x00,0x40,0x40,0x20,0x10,0x08,0x07,0x08,0x10,0x20,0x60,0x20,0x00,0x00,0x00,
0x40,0x20,0x10,0x0c,0x03,0x04,0x58,0x20,0x10,0x0c,0x03,0x0c,0x30,0x60,0x20,0x00,
};

 unsigned char code hui1[] =          // 数据表
{
/*-- ID:4,字符:"合",ASCII编码:BACF,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x80,0x80,0x40,0x20,0x50,0x48,0x44,0x43,0x44,0x48,0x50,0x20,0x40,0xc0,0x40,0x00,
0x00,0x00,0x00,0xfe,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0xff,0x02,0x00,0x00,0x00,
};

 unsigned char code hui2[] =          // 数据表
{
/*-- ID:5,字符:"达",ASCII编码:B4EF,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x40,0x42,0xcc,0x00,0x20,0x20,0x20,0xa0,0x7f,0x20,0x20,0x20,0x30,0x20,0x00,0x00,
0x40,0x20,0x1f,0x20,0x50,0x48,0x46,0x41,0x40,0x41,0x42,0x4c,0x58,0x60,0x20,0x00,
};


 unsigned char code ke[] =          // 数据表
{
/*-- ID:4,字符:"科",ASCII编码:BFC6,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x24,0x24,0x24,0xa4,0xfe,0xa3,0x22,0x00,0x24,0x48,0x00,0xff,0x00,0x80,0x00,0x00,
0x10,0x08,0x06,0x01,0xff,0x00,0x01,0x02,0x02,0x02,0x02,0xff,0x01,0x01,0x01,0x00,
};

 unsigned char code ji[] =          // 数据表
{
/*-- ID:5,字符:"技",ASCII编码:BCBC,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x10,0x10,0x10,0xff,0x10,0x10,0x88,0x88,0x88,0xff,0x88,0x88,0x8c,0x08,0x00,0x00,
0x04,0x44,0x82,0x7f,0x01,0x80,0x81,0x46,0x28,0x10,0x28,0x26,0x41,0xc0,0x40,0x00,
};

 unsigned char code you[] =          // 数据表
{
/*-- ID:6,字符:"有",ASCII编码:D3D0,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x04,0x04,0x04,0x84,0xe4,0x3c,0x27,0x24,0x24,0x24,0x24,0xf4,0x24,0x06,0x04,0x00,
0x04,0x02,0x01,0x00,0xff,0x09,0x09,0x09,0x09,0x49,0x89,0x7f,0x00,0x00,0x00,0x00,
};
 
 unsigned char code xian[] =          // 数据表
{
/*-- ID:7,字符:"限",ASCII编码:CFDE,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x00,0xfe,0x02,0x22,0xda,0x06,0x00,0xfe,0x92,0x92,0x92,0x92,0xff,0x02,0x00,0x00,
0x00,0xff,0x08,0x10,0x08,0x07,0x00,0xff,0x42,0x24,0x08,0x14,0x22,0x61,0x20,0x00,
};

 unsigned char code gong[] =          // 数据表
{
/*-- ID:8,字符:"公",ASCII编码:B9AB,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x00,0x00,0x80,0x40,0x30,0x0c,0x00,0xc0,0x06,0x18,0x20,0x40,0x80,0x80,0x80,0x00,
0x01,0x01,0x00,0x30,0x28,0x24,0x23,0x20,0x20,0x28,0x30,0x60,0x00,0x01,0x00,0x00,
};

 unsigned char code si[] =          // 数据表
{
/*-- ID:9,字符:"司",ASCII编码:CBBE,对应字:宽x高=16x16,画布:宽W=16 高H=16,共32字节*/
0x10,0x10,0x92,0x92,0x92,0x92,0x92,0x92,0xd2,0x9a,0x12,0x02,0xff,0x02,0x00,0x00,
0x00,0x00,0x3f,0x10,0x10,0x10,0x10,0x10,0x3f,0x00,0x40,0x80,0x7f,0x00,0x00,0x00
};






//extern unsigned char code ci[] =          // 数据表
//{
//      0x00,0x00,0x00,0xFE,0x42,0xC2,0x42,0x42, //尺
//      0x42,0x42,0x42,0xFF,0x02,0x00,0x00,0x00,
//      0x80,0x40,0x30,0x0F,0x00,0x01,0x02,0x04,
//      0x08,0x10,0x20,0x20,0x40,0xC0,0x40,0x00,
//};

//extern unsigned char code chun[] =          // 数据表
//{
//      0x10,0x10,0x10,0x10,0x90,0x10,0x10,0x10,	// "寸",
//      0x10,0xFF,0x10,0x10,0x10,0x18,0x10,0x00,
//      0x00,0x00,0x00,0x00,0x00,0x03,0x00,0x40,
//      0x80,0x7F,0x00,0x00,0x00,0x00,0x00,0x00
//};

//extern unsigned char code bing[] =          // 数据表	   并
//{
//      0x00,0x00,0x08,0x09,0x0A,0xFE,0x08,0x08,
//      0x08,0x0C,0xFA,0x0B,0x08,0x88,0x00,0x00,
//      0x01,0x01,0x81,0x41,0x31,0x0F,0x01,0x01,
//      0x01,0x01,0xFF,0x01,0x01,0x01,0x01,0x00,
//};

//extern unsigned char code chuan1[] =          // 数据表
//{
//      0x00,0x00,0x3C,0x24,0x24,0x24,0x24,0xFF, //串
//      0x24,0x24,0x24,0x24,0x3E,0x84,0x00,0x00,
//      0x00,0x1F,0x09,0x09,0x09,0x09,0x09,0xFF,
//      0x09,0x09,0x09,0x09,0x09,0x1F,0x01,0x00,
//};

//extern unsigned char code kou[] =          // 数据表
//{
//      0x00,0x00,0xFC,0x04,0x04,0x04,0x04,0x04, //口
//      0x04,0x04,0x04,0x04,0xFE,0x04,0x00,0x00,
//      0x00,0x00,0x3F,0x10,0x10,0x10,0x10,0x10,
//      0x10,0x10,0x10,0x10,0x3F,0x00,0x00,0x00,
//};

//extern unsigned char code shi[] =          // 数据表   视
//{
//      0x10,0x10,0x91,0xD6,0x30,0x10,0xFE,0x02,
//      0x02,0xFA,0x02,0x02,0xFF,0x02,0x00,0x00,
//      0x02,0x01,0x00,0xFF,0x01,0x82,0x47,0x30,
//      0x0C,0x03,0x3E,0x40,0x47,0x40,0x70,0x00,
//};

//extern unsigned char code qu[] =          // 数据表   区
//{
//      0x00,0x00,0xFE,0x02,0x0A,0x12,0x22,0x42,
//      0x82,0x42,0x22,0x12,0x1B,0x02,0x00,0x00,
//      0x00,0x00,0x7F,0x40,0x48,0x44,0x42,0x41,
//      0x40,0x41,0x42,0x46,0x4C,0x60,0x40,0x00,
//};
//extern unsigned char code dian2[] =          // 数据表   点
//{
//      0x00,0x00,0xC0,0x40,0x40,0x40,0x7F,0x48,
//      0x48,0x48,0x48,0xE8,0x4C,0x08,0x00,0x00,
//      0x80,0x40,0x37,0x04,0x14,0x64,0x04,0x14,
//      0x24,0x64,0x04,0x17,0x20,0xC0,0x00,0x00,
//};

//extern unsigned char code zhen2[] =          // 数据表   阵
//{
//      0x00,0xFE,0x22,0x5A,0x86,0x04,0x84,0xE4,
//      0x9F,0x84,0xF4,0x84,0xC4,0x86,0x04,0x00,
//      0x00,0xFF,0x04,0x08,0x07,0x08,0x08,0x08,
//      0x08,0x08,0xFF,0x08,0x08,0x0C,0x08,0x00,
//};
// 汉字表：
