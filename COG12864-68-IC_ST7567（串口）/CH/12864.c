...
{   
 ...
 write_cmd_dat(0,0x16); //set reference voltage //改变此值可以改变效果（对比度）  
...
void Yehui_UI()
{
	...
	uchar i; 
	const unsigned char* hanzi_array[]  = {guang, don, ye, hui,hui1,hui2, ke};
	const unsigned char* hanzi_array2[] = {ji, you, xian, gong, si};
	lcd_kuang();

	for ( i = 0; i < 7; i++) {
		display_hanzi_fan(hanzi_array[i], 2, 8 + i * 16);
	}

	for ( i = 0; i < 5; i++) {
		display_hanzi_fan(hanzi_array2[i], 4, 20 + i * 16);
	}
...
//	disp_ascii0816("+86136 2249 6694",4,1);
	    disp_ascii0816("Shenzhen Zhong", 1,8);
	disp_ascii0816("heda Technology",3,5);
	disp_ascii0816("Co.,Ltd.", 5,30);
}	
