...
//uchar ADJUST=23;	//对比度寄存器初始值	，vop调整设置  当前设置数值VDD=3.0V,VOP=13=8.0V
uchar ADJUST=43;	//对比度寄存器初始值	，vop调整设置  当前设置数值VDD=3.0V,VOP=13=8.0V
...
void InitST7567(void)
{
	...
	ST7567_WriteComd(0xf1);	//The Booster Ratio (Double Byte Command)设定倍压数,两字节
 	Delay_Xms(2);
	ST7567_WriteComd(0x03);	//"00"最大4倍压,"01"最大5倍压,"03"最大6倍压
 	Delay_Xms(2);
...
void DispPic(unsigned char *dispdata)
{
    ...
    for(page = 0xb0; page < 0xb8; page++)
    {
        ...
        ST7567_WriteComd(0x10);   // 列地址高位
        ST7567_WriteComd(0x00);   // 列地址低位

        // 每页写 128 列数据
        for(i = 0; i < 128; i++)
        {
            ST7567_WriteData(dispdata[j]);
            j++;
        }
    }
}
...
void yehui_UI()
{
	...
	char *words3[] = {C, O, G, one,two,eight,xing,six,four};
...
//	}
	...
	{
	    unsigned char col_start = 25;  // 第一个字符的起始列
...
{
	unsigned char k;
	char *words6[] = {S,hh,ee,nn,zz,hh,ee,nn,Z,hh,oo,nn,T,ee,};//Shenzhen Zhongheda Technology Co., Ltd.
	char *words7[] = {hh,nn,oo,ll,oo,gg,yy,KG,C,oo,DIAN,DH,L,tt,D};
