...
{ 
 ...
 {	
  GotoXY(j,0);// 设置当前页 j,列地址为 0
   for(i=0;i<128/2;i++) // 每页写 64 次,每次写两个字节
  {
   write_cmd_dat(1,fst);
   write_cmd_dat(1,snd);
  }
...
{
	uchar i; 
	const unsigned char* hanzi_array[] = {guang, don, ye, hui, ke,ci,chun};
	const unsigned char* hanzi_array2[] = {ji, you, xian, gong, si};
	
	for ( i = 0; i < 7; i++) {
		display_hanzi_fan(hanzi_array[i], 0, 10 + i * 16);
	}
	...
	disp_ascii0816("128*64 Dots",4,20);
	disp_ascii0816("IC:ST7565R",  6,25);
