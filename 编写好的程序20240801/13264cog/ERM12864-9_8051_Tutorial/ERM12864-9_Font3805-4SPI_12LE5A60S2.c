...
// Create    	: 	JAVEN LIU
// Date      	: 	2023-05-29
// Drive IC  	: 	LCD:ST7567	    FONT:3805
// INTERFACE 	: 	LCD:4SPI   FONT:4SPI
// MCU 	 		: 	ST12LE5A60S2
// VDD		 	: 	3.3V       
//----------------------------------------------------------------------
...
//FONT IO
sbit  FSI 	=   P2^0;    // Font chip Serial data input
sbit  FSO 	=   P2^1;    // Font chip Serial data output
sbit  FSCK	=   P2^2;    // Font chip Serial clock input
sbit  FCS 	=  	P2^3;    //  Font  Chip enable input
...
void Display_One_Chinese_16X16(unsigned char page,unsigned char column,unsigned char num,unsigned char ft1[]);
void Display_One_Chinese_8X16(unsigned char page,unsigned char column,unsigned char num,unsigned char ft1[]);
...
"Íæ×ªÐÂÒº¾§³±Á÷ ½âÎöÏÔÊ¾Æ÷¶àÆÁÆ´½Ó¸ÅÄî"

" WWW.BIYDISPLAY.COM"

"ÌáÒª£º ÏÔÊ¾Æ÷½øÈëÁË16:10¡¢16:9¿íÆÁÊ±´úÖ®ºó£¬³ß´çÒÑÃ÷ÏÔÃ»ÓÐÁË¶à´óµÄÏÞÖÆÁË¡£¶ÔÓÚÒ»¸öÓÎÏ·Íæ¼ÒÀ´Ëµ£¬¹ºÂòÒ»¸ö30´çµÄÒº¾§ÏÔÊ¾Æ÷ÒÑ¾­²»ÔÙÊÇ³¬ÈËÏëÏóµÄÎÊÌâÁË¡£µ«Ïà±È30´çµÄÏÔÊ¾Æ÷À´Ëµ£¬¹ºÂòË«ÆÁ»òÕß¶àÆÁ27´çÒº¾§ÏÔÊ¾Æ÷£¬³É±¾ÉÏÓÐ×Å²»Ð¡ÓÅÊÆ¡£¶àÆÁÆ´½ÓÕâ¸öÇ°ÎÀµÄÐÂ¸ÅÄîµÄ³öÏÖÊ¹µÃÍæ¼Ò²»½ö½ö¾ÖÏÞÓÚÒ»Ì¨ÏÔÊ¾Æ÷·Ö±æÂÊµÄÕðº³Ð§¹û¡£"
"Ò»¡¢¶àÆÁÆ´½Ó¸ÅÄîÒÔ¼°ÊÊºÏÆ´½ÓµÄÏÔÊ¾Æ÷ÌØµã"
"Ò»¿îºÃµÄÓÎÏ·ÐèÒª¸ü¸ßµÄ·Ö±æÂÊ£¬ËùÒÔ¶àÆÁÆ´½ÓµÄ¸ÅÄî¶ÔÓÎÏ··¢ÉÕÍæ¼Ò¶øÑÔ²¢²»Ä°Éú¡£¼øÓÚ¹ºÂò×¨Òµ³¬¸ß·Ö±æÂÊÏÔÊ¾Æ÷¼Û¸ñ°º¹ó£¬ºÜ¶àÓÎÏ··¢ÉÕÍæ¼ÒÏà³öÁËÒ»¸ö½â¾öµÄ·½°¸£¬¾ÍÊÇ¾ÍÊÇ²ÉÓÃ¶àÌ¨ÏàÍ¬³ß´çµÄÒº¾§ÏÔÊ¾Æ÷£¬Ò»¶¨µÄ¼¼ÊõÆ´½ÓÆðÀ´£¬Ê¹Æä¼òµ¥ÊµÏÖ³¬´óÆÁÊä³öÐ§¹û£¬Õâ¾ÍÊÇ´ó¼ÒÍ¨³£ËùËµµÄ¶àÆÁÆ´½ÓÁË¡£"

//   	ÓÉºº×ÖÄÚÂë SS[0] S[1] ......
//	 	¼ÆËãµÄÔÚGT23L16S2WÖÐµÄµãÕóÊý¾ÝµØÖ·
//  ²ÎÊý:
//  	ss 		ºº×ÖGB2312ÄÚÂëÖ¸Õë
//		type 	ºº×ÖÀàÐÍ
//  ·µ»Ø:
//		ºº×ÖÔÚÐ¾Æ¬ÖÐµÄµãÕóÊý¾ÝÆðÊ¼µØÖ·
//--------------------------------------------------
unsigned long  GB2312_Addr(uchar MSB,uchar LSB)
{	unsigned long	Address;	
	unsigned long 	GB_1516_START_ADDR = 0;	
	unsigned long 	GB_816_START_Address = 0x3b7d0;   
	if(MSB ==0xA9 && LSB >=0xA1)       	 												//·ûºÅÇø,È«½Ç
		Address = (282 + (LSB - 0xA1 ))*32+GB_1516_START_ADDR;

		else if(MSB >=0xA1 && MSB <= 0xA3 && LSB >=0xA1)       	 						//·ûºÅÇø,È«½Ç
			Address =( (MSB - 0xA1) * 94 + (LSB - 0xA1))*32+ GB_1516_START_ADDR;

						
	/*		else if(MSB >=0xB0 && MSB <= 0xF7 && LSB >=0xA1)       	 					//GBKË«×Ö½Ú2Çø 6768,³Ë·¨Òç³ö£¬ËùÒÔÏÂÃæ·Ö¿ª³Ë
				Address = ((MSB - 0xB0) * 94 + (LSB - 0xA1)+ 846)*32+ GB_1516_START_ADDR;*/


		else if(MSB >=0xB0 && MSB <= 0xF7 && LSB >=0xA1)       	 					//GBKË«×Ö½Ú2Çø 6768,³Ë·¨Òç³ö£¬ËùÒÔÏÂÃæ·Ö¿ª³Ë
				Address =GB_1516_START_ADDR+((MSB - 0xB0)*752) +((MSB - 0xB0)*752) +((MSB - 0xB0)*752) +((MSB - 0xB0)*752) + (LSB - 0xA1)*32+ 846*32 ;
  
 	if(MSB ==0xAA && LSB <=0xFF)       	 						//·ûºÅÇø,
		Address =( (MSB - 0xAA) + (LSB - 0xA1)*16)+ GB_816_START_Address;	
		else if(MSB ==0xAB && LSB >=0xA1  && LSB <=0xC0)       	 						//·ûºÅÇø,
			Address =( (MSB - 0xAB) + (LSB - 0xA1)+95)*16+ GB_816_START_Address;
...
void Display_GB2321(void)
{   uchar count,page=0,col=32,h,m,n;
	uchar i=0xa1;uchar j=0xa1;						//16µãºº×Ö ¡¡¡±  µÄGB2312ÄÚÂë 
   	uchar dot[32];			  							//´æ´¢ºº×Ö ¡¡¡±  µÄµãÕóÊý¾Ý¡£
   	unsigned long a_dot_address;
  
 	while(i <= 0xF7 )
		{
				 	h=i&0xf0;	
					if(h==0xa0)(m=0xaa,n=0xc1);
				 	else if(h==0xb0)(m=0xaa,n=0xc2);
				 	else if(h==0xc0)(m=0xaa,n=0xc3);	
				 	else if(h==0xd0)(m=0xaa,n=0xc4);
				 	else if(h==0xe0)(m=0xaa,n=0xc5);
				 	else if(h==0xf0)(m=0xaa,n=0xc6);				



								a_dot_address = GB2312_Addr(m,n);
								for( count = 0; count < 16; count++ )   		//µÃµ½¡¡¡± µãÕóÊý¾Ý
   									{
										dot[count] =  readbyte(a_dot_address+count);
									}
								
						   		Display_One_Chinese_8X16(0,0,0,dot);

				 	h=i&0x0f;
					if(h==0x00)(m=0xaa,n=0xb0);
					else if(h==0x01)(m=0xaa,n=0xb1);
				 	else if(h==0x02)(m=0xaa,n=0xb2);
				 	else if(h==0x03)(m=0xaa,n=0xb3);	
				 	else if(h==0x04)(m=0xaa,n=0xb4);
				 	else if(h==0x05)(m=0xaa,n=0xb5);
				 	else if(h==0x06)(m=0xaa,n=0xb6);	
				 	else if(h==0x07)(m=0xaa,n=0xb7);
				 	else if(h==0x08)(m=0xaa,n=0xb8);
				 	else if(h==0x09)(m=0xaa,n=0xb9);	
				 	else if(h==0x0a)(m=0xaa,n=0xc1);
				 	else if(h==0x0b)(m=0xaa,n=0xc2);
				 	else if(h==0x0c)(m=0xaa,n=0xc3);	
				 	else if(h==0x0d)(m=0xaa,n=0xc4);
				 	else if(h==0x0e)(m=0xaa,n=0xc5);
				 	else if(h==0x0f)(m=0xaa,n=0xc6);	
								...
								for( count = 0; count < 16; count++ )   		//µÃµ½¡¡¡± µãÕóÊý¾Ý
						   		...
						   		Display_One_Chinese_8X16(0,8,0,dot);


				 	h=j&0xf0;
					if(h==0x00)(m=0xaa,n=0xb0);
					else if(h==0x10)(m=0xaa,n=0xb1);
				 	else if(h==0x20)(m=0xaa,n=0xb2);
				 	else if(h==0x30)(m=0xaa,n=0xb3);	
				 	else if(h==0x40)(m=0xaa,n=0xb4);
				 	else if(h==0x50)(m=0xaa,n=0xb5);
				 	else if(h==0x60)(m=0xaa,n=0xb6);	
				 	else if(h==0x70)(m=0xaa,n=0xb7);
				 	else if(h==0x80)(m=0xaa,n=0xb8);
				 	else if(h==0x90)(m=0xaa,n=0xb9);	
				 	else if(h==0xa0)(m=0xaa,n=0xc1);
				 	else if(h==0xb0)(m=0xaa,n=0xc2);
				 	else if(h==0xc0)(m=0xaa,n=0xc3);	
				 	else if(h==0xd0)(m=0xaa,n=0xc4);
				 	else if(h==0xe0)(m=0xaa,n=0xc5);
								...
								for( count = 0; count < 16; count++ )   		//µÃµ½¡¡¡± µãÕóÊý¾Ý
						   		...
						   		Display_One_Chinese_8X16(0,16,0,dot);
					...
					if(h==0x00)(m=0xaa,n=0xb0);
								...
								for( count = 0; count < 16; count++ )   		//µÃµ½¡¡¡± µãÕóÊý¾Ý
						   		...
						   		Display_One_Chinese_8X16(0,24,0,dot);


   			if((i>=0xaa)&&(i<=0xab))
				{a_dot_address = GB2312_Addr(i,j);				 //µÃµ½¡¡¡± µãÕóÊý¾ÝµÄÆðÊ¼µØÖ·
					...
					for( count = 0; count < 16; count++ )   		//µÃµ½¡¡¡± µãÕóÊý¾Ý
   					...
   					Display_One_Chinese_8X16(page,col,0,dot);			   //ÔÚ×ø±ê£¨X£¬Y£©£¬ÏÔÊ¾ºº×Ö¡°¡¡±
	  				Delay(100);
					col+=8;
					if(col>=128)
					{	page+=2;col=0;
			...
			else{a_dot_address = GB2312_Addr(i,j);				 //µÃµ½¡¡¡± µãÕóÊý¾ÝµÄÆðÊ¼µØÖ·

					for( count = 0; count < 32; count++ )   		//µÃµ½¡¡¡± µãÕóÊý¾Ý
   					{
						dot[count] =  readbyte(a_dot_address+count);
					}

   					Display_One_Chinese_16X16(page,col,0,dot);			   //ÔÚ×ø±ê£¨X£¬Y£©£¬ÏÔÊ¾ºº×Ö¡°¡¡±
	  				Delay(100);
					col+=16;
					if(col>=128)
					{	page+=2;col=0;
		...
		if(j==0xff)
			{i+=1;j=0xa1;}
				if(i==0xa4)(i=0xa9);
				if((i==0xab)&&(j==0xc1)){i=0xb0;j=0xa1;}				
...
void Display_GB2312_String(uchar x,uchar y)
{uchar code text[]={"Òº¾§Ä£¿é×Ö¿âÑÝÊ¾"};
	uchar code 	text1[]={"ERM12864-9"};
		uchar code 	text2[]={"BUYDISPLAY.COM"};
		uchar code clr[16]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	uchar i= 0,j;
	uchar fontbuf[32];
  	unsigned long fontaddr,GB_1516_START_ADDR=0;		
	while(text[i]>0x00)
	{
			fontaddr = GB2312_Addr(text[i],text[i+1]);  				 //µÃµ½¡¡¡± µãÕóÊý¾ÝµÄÆðÊ¼µØÖ·
			GT_Read_1_n(fontaddr,fontbuf,32 );							  		//µÃµ½¡¡¡± µãÕóÊý¾Ý			
			Display_One_Chinese_16X16(y,x,0,fontbuf);			   //ÔÚ×ø±ê£¨X£¬Y£©£¬ÏÔÊ¾ºº×Ö¡°¡¡±
			i+=2;
			x+=16;	if((x>112)&&(x<=128))(y+=3,x=8);
		
	}
	
	x=00;y=3;i=0;
	while(text1[i]>0x00)
	{	j=(text1[i]); j=j+0xa0-0x20;
		if(j==0xa0)Display_One_Chinese_8X16(y,x,0,clr);
		else{
			fontaddr = GB2312_Addr(0xaa,j);  				 //µÃµ½¡¡¡± µãÕóÊý¾ÝµÄÆðÊ¼µØÖ·
			GT_Read_1_n(fontaddr,fontbuf,16 );							  		//µÃµ½¡¡¡± µãÕóÊý¾Ý	
   			Display_One_Chinese_8X16(y,x,0,fontbuf);			   //ÔÚ×ø±ê£¨X£¬Y£©£¬ÏÔÊ¾ºº×Ö¡°¡¡±
			}
			i+=1;
			x+=8;
	}
...
{
uint i= 0; uchar x=0,y=0,j; 
	uchar fontbuf[32];
	uchar code clr[32]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	...
	while((text[i])>0x00)
	{
		if((text[i]>=0xb0&&text[i]<=0xf7&&text[i+1]>=0xa1)||(text[i]>=0xA1 && text[i] <= 0xA3 && text[i+1] >=0xA1)||(text[i]==0xA9 && text[i+1] >=0xA1))
		{
			fontaddr = GB2312_Addr(text[i],text[i+1]);  				 //µÃµ½¡¡¡± µãÕóÊý¾ÝµÄÆðÊ¼µØÖ·
			GT_Read_1_n(fontaddr,fontbuf,32 );							  		//µÃµ½¡¡¡± µãÕóÊý¾Ý			
			Display_One_Chinese_16X16(y,x,0,fontbuf);			   //ÔÚ×ø±ê£¨X£¬Y£©£¬ÏÔÊ¾ºº×Ö¡°¡¡±
			...
			if((x+16>128)||(x==128))
			{y+=2;x=0;
			if(y==8){y=0;x=0;delay_ms(500); ClearScreen();}
			}
		}
		

		else if((text[i]>0)&&(text[i]<=0x60))
		{	j=(text[i]); j=j+0xa0-0x20;
			if(j==0xa0)Display_One_Chinese_8X16(y,x,0,clr);
