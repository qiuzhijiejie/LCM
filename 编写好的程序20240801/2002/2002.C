//LCM2002A标准字符点阵型液晶显示模块的演示程序[C51编程语言][MCS51模拟口线方式]
//应用产品: LCM2002A标准字符点阵型液晶显示模块
//  本演示程序适用于LCM2002A液晶显示模块与MCS51系列单片机采用MCS51模拟口线的
//硬件连线方式。
//  本演示程序包括HD44780兼容芯片的MCS51模拟口线方式子程序集,LCM2002A系列标准字符
//点阵型液晶显示模块的接口子程序集,HD44780兼容芯片的液晶显示控制器的通用子程序集,
//以及LCM2002A系列标准字符点阵型液晶显示的演示子程序.
//  本演示的内容为,在20列ASCII字符X2行的液晶显示屏上显示"SMC2002A",
...
//1.字符串演示: 在(6,0)字符位置显示"LCM2002A"
    putstrxy(6,0,"LCM2002A");           //在(6,0)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//2.字符串演示: 在(0,1)字符位置显示"WWW.ORIENTAL-LCD.COM"
