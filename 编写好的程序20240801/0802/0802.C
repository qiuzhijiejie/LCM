//SMC0802B标准字符点阵型液晶显示模块的演示程序[C51编程语言][MCS51模拟口线方式]
//应用产品: SMC0802B标准字符点阵型液晶显示模块
//  本演示程序适用于SMC0802B液晶显示模块与MCS51系列单片机采用MCS51模拟口线的
//硬件连线方式。
//  本演示程序包括HD44780兼容芯片的MCS51模拟口线方式子程序集,SMC0802系列标准字符
//点阵型液晶显示模块的接口子程序集,HD44780兼容芯片的液晶显示控制器的通用子程序集,
//以及SMC0802系列标准字符点阵型液晶显示的演示子程序.
//  本演示的内容为,在8列ASCII字符X2行的液晶显示屏上显示"SMC0802B","SUNMAN",所有数
//字0..9依次全屏显示一遍,显示清屏,所有英文字母依次显示,;依次读字符并加1后送回LCD
//显示.
//-------------------------------------------------------------------------------
//创建日期: 2009.08.13
//软件环境: VW For Windows
...
//参考网页: http:/www.sunman.cn/lcm/product/SMC0802B.html
//-------------------------------------------------------------------------------
#include <reg51.h>
#include <intrins.h>
//以下为演示程序的函数定义
...
void lcdreset(void);                    //SMC0802系列液晶显示控制器初始化子程序
...
//PIN16:BLK  [背光源负极]---------------VSS
//请参见http://download.sunman.cn/lcm/product/1/SMC0802B.pdf
...
//1.字符串演示: 在(0,0)字符位置显示"LCM0802B"
    putstrxy(0,0,"LCM0802B");           //在(0,0)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//2.字符串演示: 在(1,1)字符位置显示"Oriental"
    putstrxy(0,1,"Oriental");             //在(0,1)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS
...
//5.字符演示: 依次读字符并加1后送回LCD显示.
    for(CXPOS=0,CYPOS=0,i=0;i<16;i++)   //初始位置(0,0)
...
//8字符X2行的SMC0802系列标准字符点阵型液晶显示模块的接口程序.
//-------------------------------------------------------------------------------
...
void charlcdpos(void)                   //设置(CXPOS,CYPOS)字符位置的DDRAM地址
{
    CXPOS&=0X07;                        //X位置范围(0到7)
    CYPOS&=0X01;                        //Y位置范围(0到1)
    if(CYPOS==0)                        //(第一行)X: 第0----7个字符
