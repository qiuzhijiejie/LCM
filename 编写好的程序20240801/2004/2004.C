//SMC2004A标准字符点阵型液晶显示模块的演示程序[ASM51编程语言][MCS51模拟口线方式]
//应用产品: SMC2004A标准字符点阵型液晶显示模块
//  本演示程序适用于SMC2004A液晶显示模块与MCS51系列单片机采用MCS51模拟口线的
//硬件连线方式。
//  本演示程序包括HD44780兼容芯片的MCS51模拟口线方式子程序集,SMC2004系列标准字符
//点阵型液晶显示模块的接口子程序集,HD44780兼容芯片的液晶显示控制器的通用子程序集,
//以及SMC2004系列标准字符点阵型液晶显示的演示子程序.
//  本演示的内容为,在20列ASCII字符X4行的液晶显示屏上显示"SMC2004A",
//"WWW.SUNMAN.CN","TEL:073184167806","FAX:073184167807"所有数字0..9依次全屏显示
...
//参考网页: http:/www.sunman.cn/lcm/product/SMC2004A.html
//-------------------------------------------------------------------------------
#include <reg51.h>
#include <intrins.h>
//以下为演示程序的函数定义
...
void lcdreset(void);                    //SMC2004系列液晶显示控制器初始化子程序
...
//PIN16:BLK  [背光源负极]---------------VSS
//请参见http://download.sunman.cn/lcm/product/1/SMC2004A.pdf
...
//1.字符串演示: 在(6,0)字符位置显示"SMC2004A"
    putstrxy(6,0,"SMC2004A");           //在(6,0)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//2.字符串演示: 在(4,1)字符位置显示"WWW.SUNMAN.CN"
    putstrxy(4,1,"WWW.SUNMAN.CN");      //在(4,1)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//3.字符串演示: 在(2,2)字符位置显示"TEL:073184167806"
    putstrxy(2,2,"TEL:073184167806");   //在(2,2)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//4.字符串演示: 在(2,3)字符位置显示"FAX:073184167807"
    putstrxy(2,3,"FAX:073184167807");   //在(2,3)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS
...
//20字符X4行的SMC2004系列标准字符点阵型液晶显示模块的接口程序.
//-------------------------------------------------------------------------------
...
void charlcdpos(void)                   //设置(CXPOS,CYPOS)字符位置的DDRAM地址
{
    if(CXPOS>=20) CXPOS=0;              //X位置范围(0到15)
    CYPOS&=0X03;                        //Y位置范围(0到3)
