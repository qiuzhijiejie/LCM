...
{
	u8 c,i,k,m,t=4,size2,data,DATA=0;
	...
	for(i=0;i<size2;i++)
	{
		if(sizey==16)
		{
			data=ascii_1608[c][i];//8x16 ASCII码
		}
		else if(sizey==24)
		{
			data=ascii_2412[c][i];//12x24 ASCII码
		}
		else if(sizey==32)
		{
			data=ascii_3216[c][i];//16x32 ASCII码
		}
...
//    返回值：  无
//******************************************************************************
void OLED_DrawBMP(u8 x,u8 y,u16 length,u8 width,const u8 BMP[])
{
	u16 num;
	...
	OLED_CS_Clr();
	MYDMA_Config(DMA1_Channel3,(u32)&SPI1->DR,(u32)BMP,num);
	SPI_I2S_DMACmd(SPI1,SPI_I2S_DMAReq_Tx,ENABLE);
	MYDMA_Enable(DMA1_Channel3);
	while(1)
	{
