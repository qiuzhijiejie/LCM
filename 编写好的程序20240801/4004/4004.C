//LCM4004A标准字符点阵型液晶显示模块的演示程序[C51编程语言][MCS51模拟口线方式]
//应用产品: LCM4004A标准字符点阵型液晶显示模块
//  本演示程序适用于LCM2402A液晶显示模块与MCS51系列单片机采用MCS51模拟口线的
//硬件连线方式。
//  本演示程序包括HD44780兼容芯片的MCS51模拟口线方式子程序集,LCM4004A系列标准字符
//点阵型液晶显示模块的接口子程序集,HD44780兼容芯片的液晶显示控制器的通用子程序集,
//以及LCM4004A系列标准字符点阵型液晶显示的演示子程序.
//  本演示的内容为,在40列ASCII字符X2行的液晶显示屏上显示"LCM4004A",
//" Oriental Technology/WWW.ORIENTAL-LCD.COM ",所有数字0..9依次全屏显示一遍,显示清屏,所有英文字母依次显示,
//依次读字符并加1后送回LCD显示.
//-------------------------------------------------------------------------------
//创建日期: 2009.08.11
//软件环境: VW For Windows
...
//参考网页: http:/www.yehuidisplay.com/lcm/product/1/LCM4002A.pdf
//-------------------------------------------------------------------------------
#include <reg51.h>
#include <intrins.h>
//以下为演示程序的函数定义
...
void lcdreset(void);                    //SMC2002系列液晶显示控制器初始化子程序
...
//PIN16:BLK  [背光源负极]---------------VSS
//请参见http:/www.yehuidisplay.com/lcm/product/1/LCM4004A.pdf
...
//1.字符串演示: 在(16,0)字符位置显示"LCM4004A"
    putstrxy(16,0,"LCM4004A");           //在(16,0)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//2.字符串演示: 在(0,1)字符位置显示"Oriental Technology/WWW.ORIENTAL-LCD.COM"
    putstrxy(0,1,"Oriental Technology/WWW.ORIENTAL-LCD.COM");      //在(0,1)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS
...
//20字符X2行的LCM2002系列标准字符点阵型液晶显示模块的接口程序.
//-------------------------------------------------------------------------------
...
void charlcdpos(void)                   //设置(CXPOS,CYPOS)字符位置的DDRAM地址
{
    if(CXPOS>=40) CXPOS=0;              //X位置范围(0到39)
    CYPOS&=0X01;                        //Y位置范围(0到1)
    if(CYPOS==0)                        //(第一行)X: 第0----39个字符
...
//修改人:谢建仁
//-------------------------------------------------------------------------------
void lcdreset(void)                     //LCM2402系列液晶显示控制器初始化子程序
{                                       //2402的显示模式字为0x38
