//SMG16032D标准字符点阵型液晶显示模块的演示程序[C51编程语言][MCS51并行接口方式]
//应用产品: SMG16032D标准中文字符点阵型液晶显示模块
//  本演示程序适用于SMG16032D液晶显示模块与MCS51系列单片机采用MCS51并行接口
//方式的硬件连线方式。
//  本演示程序包括ST7920兼容芯片的MCS51并行接口方式子程序集,SMG16032系列标准中文
//字符点阵型液晶显示模块的接口子程序集,ST7920兼容芯片的液晶显示控制器的通用子程序
//集,以及SMG16032系列标准中文字符点阵型液晶显示的演示子程序.
//  本演示的内容为,在20列ASCII字符(10列中文)X2行的液晶显示屏上显示" SMG16032D ",
...
//PIN19:NC   [空脚]
//PIN20:NC   [空脚]
//请参见http://download.sunman.cn/lcm/product/1/SMG16032D.pdf
...
//1.字符串演示: 在(4,0)字符位置显示" SMG16032D "
    putstrxy(4,0," SMG16032D ");       //在(4,0)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//2.字符串演示: 在(2,1)字符位置显示"中文字符图形点阵"
    putstrxy(2,1,"中文字符图形点阵");   //在(2,1)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//3.字符串演示: 在(4,0)字符位置显示"液晶显示模块"
    putstrxy(4,0,"液晶显示模块");       //在(4,0)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//4.字符串演示: 在(2,1)字符位置显示"长沙太阳人SUNMAN"
    putstrxy(2,1,"长沙太阳人SUNMAN");   //在(2,1)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS
...
//20ASCII字符X2行的SMG16032系列标准中文字符点阵型液晶显示模块的接口程序.
//-------------------------------------------------------------------------------
