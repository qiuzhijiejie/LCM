//LCM2402A标准字符点阵型液晶显示模块的演示程序[C51编程语言][MCS51模拟口线方式]
//应用产品: LCM2002A标准字符点阵型液晶显示模块
//  本演示程序适用于LCM2402A液晶显示模块与MCS51系列单片机采用MCS51模拟口线的
//硬件连线方式。
//  本演示程序包括HD44780兼容芯片的MCS51模拟口线方式子程序集,LCM2402A系列标准字符
//点阵型液晶显示模块的接口子程序集,HD44780兼容芯片的液晶显示控制器的通用子程序集,
//以及LCM2402A系列标准字符点阵型液晶显示的演示子程序.
//  本演示的内容为,在20列ASCII字符X2行的液晶显示屏上显示"LCM2402A",
...
//参考网页: http:/www.yehuidisplay.com/lcm/product/1/LCM2402A.pdf
//-------------------------------------------------------------------------------
#include <reg51.h>
#include <intrins.h>
//以下为演示程序的函数定义
...
//PIN16:BLK  [背光源负极]---------------VSS
//请参见http:/www.yehuidisplay.com/lcm/product/1/LCM2402A.pdf
...
//1.字符串演示: 在(8,0)字符位置显示"LCM2402A"
    putstrxy(8,0,"LCM2402A");           //在(8,0)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//2.字符串演示: 在(2,1)字符位置显示"WWW.ORIENTAL-LCD.COM"
    putstrxy(2,1,"WWW.ORIENTAL-LCD.COM");      //在(2,1)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS
...
void charlcdpos(void)                   //设置(CXPOS,CYPOS)字符位置的DDRAM地址
{
    if(CXPOS>=24) CXPOS=0;              //X位置范围(0到23)
    CYPOS&=0X01;                        //Y位置范围(0到1)
    if(CYPOS==0)                        //(第一行)X: 第0----23个字符
