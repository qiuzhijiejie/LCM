//LCM1602A标准字符点阵型液晶显示模块的演示程序[C51编程语言][MCS51模拟口线方式]
//应用产品:  LCM1602A标准字符点阵型液晶显示模块
//  本演示程序适用于LCM1602A液晶显示模块与MCS51系列单片机采用MCS51模拟口线的
//硬件连线方式。
//  本演示程序包括HD44780兼容芯片的MCS51模拟口线方式子程序集,LCM1602系列标准字符
//点阵型液晶显示模块的接口子程序集,HD44780兼容芯片的液晶显示控制器的通用子程序集,
//以及LCM1602系列标准字符点阵型液晶显示的演示子程序.
//  本演示的内容为,在16列ASCII字符X2行的液晶显示屏上显示"LCM1602A",
//"WWW.WWW.YEHUIDISPLAY.COM.CN",所有数字0..9依次全屏显示一遍,显示清屏,所有英文字母依次显示,
//依次读字符并加1后送回LCD显示.
//-------------------------------------------------------------------------------
//创建日期: 2009.08.11
//软件环境: VW For Windows
...
//参考网页: http:/www.yehuidisplay.com/lcm/product/1/LCM1602A.pdf
//-------------------------------------------------------------------------------
#include <reg51.h>
#include <intrins.h>
//以下为演示程序的函数定义
...
//PIN16:BLK  [背光源负极]---------------VSS
//请参见http://download.sunman.cn/lcm/product/1/LCM1602A.pdf
...
//1.字符串演示: 在(4,0)字符位置显示"LCM1602A"
    putstrxy(4,0,"LCM1602A");           //在(4,0)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS

//2.字符串演示: 在(0,1)字符位置显示"ORIENTAL-LCD.COM"
    putstrxy(0,1,"ORIENTAL-LCD.COM");      //在(0,1)位置开始显示字符串
    exdelay();                          //延时约300mS
    exdelay();                          //延时约300mS
