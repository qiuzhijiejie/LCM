//SMG12864G标准图形点阵型液晶显示模块的演示程序[C51编程语言][MCS51模拟口线接口方式]
//应用产品:SMG12864G SMG12864A SMG12864BV SMG12864C SMG12864D3 标准图形点阵型液晶显示模块
//  本演示程序适用于SMG12864G液晶显示模块与MCS51系列单片机采用MCS51模拟口线的
//硬件连线方式。
//  本演示程序包括KS0108B兼容芯片的MCS51模拟口线接口子程序集,KS0108B兼容芯片的12864
//液晶显示模块的基本子程序,12864系列绘图子程序集,12864系列图形子程序集,12864系列
//精简中英文字库子程序集,以及SMG12864G系列标准图形点阵型液晶显示的演示子程序.
...
//参考网页: http:/www.sunman.cn/lcm/product/SMG12864G.html
//-------------------------------------------------------------------------------
#include <reg51.h>
#include <intrins.h>
//以下为演示程序的函数定义
...
//PIN20:BLK  [背光源负极]
//注:8031的晶振频率为12MHz.
//请参见http://download.sunman.cn/lcm/product/1/SMG12864G.pdf
//-------------------------------------------------------------------------------
//以下为产品接口引脚在演示程序中的预定义
...
#define ENGLISHCHARNUMBER  31      //精简版英文字符库中的英文字符的个数
...
0x00,0x00,0x70,0x1c,0x88,0x22,0x08,0x21,   //字符008:[8]
0x08,0x21,0x88,0x22,0x70,0x1c,0x00,0x00,
...
//1.图片测试: 在(2,1)位置显示32点阵Oriental图标
//    XPOS=2;
//    YPOS=1;
//    putimage(Img_sunman_32);            //写数据到当前LCDRAM地址中
//    exdelay();                          //延时约600mS

//2.字符串测试: 在(40,0)位置显示"LCM12864A"字符串.
    XPOS=40;
    YPOS=0;
    putstr("LCM12864");
    exdelay();                          //延时约600mS

//3.字符串测试: 在(36,16)位置显示"Oriental"字符串.
    XPOS=20;
    YPOS=16;
    putstr("In China");
    exdelay();                          //延时约600mS

//4.字符串测试: 在(1,32)位置显示"ORIENTAL-LCD.COM"字符串.
//    XPOS=1;
//    YPOS=32;
//    putstr("ORIENTAL-LCD.COM");
//    exdelay();                          //延时约600mS

//5.反显测试: 在(1,32)位置反显"ORIENTAL-LCD.COM"字符串.
//    XPOS=1;
//    YPOS=32;
//    CharImageReverse=1;
//    putstr("ORIENTAL-LCD.COM");
...
//6.绘图测试: RECT(0,0)-(127,63),画矩形.
    rect(0,0,127,63,1);
    exdelay();                          //延时约600mS

//7.绘图测试: POINT(6,50),画点.
    XPOS=6;
    YPOS=50;
    point(1);
    exdelay();                          //延时约600mS

//8.绘图测试: LINE(6,50)-(63,63),画线.
    line(6,50,63,63,1);
    exdelay();                          //延时约600mS

//9.绘图测试: lineto(122,50),画线.
    lineto(122,50,1);
    exdelay();                          //延时约600mS

//10.绘图测试: lineto(6,50),画线.
    lineto(6,50,1);
    exdelay();                          //延时约600mS

//11.全屏测试: 所有坐标点全部显示.
    lcdfill(0xff);                      //全显
    exdelay();  exdelay();                        //延时约600mS
	    lcdfill(0xaa);                      //全显
    exdelay(); exdelay();
	    lcdfill(0x55);                      //全显
    exdelay(); exdelay();
