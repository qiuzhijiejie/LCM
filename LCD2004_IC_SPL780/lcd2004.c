...
void LCD_SetCursor(unsigned int Line, unsigned int Column) 
{
    ...
    {
        LCD_WriteCommand(0x80 | (Column + 20));  // 第三行
    } 
    else 
    {
        LCD_WriteCommand(0xC0 | (Column + 20));  // 第四行
    }
}
...
/**
 * @brief LCD2004显示字符串
 * @param Line 0-4行 Column 0-19列 *String 输入字符""
 * @retval 无
 */
...
void displayCustomCharacter1() 
{
	...
	unsigned char pattern1[8] = {0x15, 0x0A, 0x15, 0x0A, 0x15, 0x0A, 0x15}; // 棋盘格样式
	unsigned char pattern2[8] = {0x0A, 0x15, 0x0A, 0x15, 0x0A, 0x15, 0x0A};
	unsigned char pattern3[8] = {0x1C, 0x1C, 0x1C, 0x1C, 0x00, 0x00, 0x00, 0x00}; // 代表齿轮的形状
	unsigned char pattern4[8] = {0x00, 0x0A, 0x00, 0x0A, 0x00, 0x0A, 0x00, 0x00}; // 模拟波浪形状
	unsigned char pattern5[8] = {0x1E, 0x11, 0x1F, 0x00, 0x00, 0x00, 0x00, 0x00}; // 蜡烛的形状
	unsigned char pattern6[8] = {0x04, 0x0A, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00}; // 心电图波形
	unsigned char pattern7[8] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}; 
    // 将自定义字符数组写入CGRAM
    Write_CGRAM(pattern7);
    ...
    for (row = 0; row < 4; row++) 
    {
        for (col = 0; col < 20; col++) 
        {
            ...
            if (row == 0) {
                ...
                position = 0x94 + col;   // 第3行地址 0x94 起始
            } else {
                position = 0xD4 + col;   // 第4行地址 0xD4 起始
            }
