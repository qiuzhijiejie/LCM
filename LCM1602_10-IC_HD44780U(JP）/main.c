#include <REGX52.H>
#include "LCD1602.h"
	unsigned char mode;
	unsigned char data pattern1[8] = {0x15, 0x0A, 0x15, 0x0A, 0x15, 0x0A, 0x15}; // 棋盘
	unsigned char data pattern2[8] = {0x0A, 0x15, 0x0A, 0x15, 0x0A, 0x15, 0x0A};	
	unsigned char data pattern3[8] = {0x15, 0x15, 0x15, 0x15, 0x15, 0x15, 0x15, 0x15}; //竖线
    unsigned char data pattern4[8] = {0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A}; 
    unsigned char data pattern5[8] = {0x1F, 0x00, 0x1F, 0x00, 0x1F, 0x00, 0x1F, 0x00};//横线 
	unsigned char data pattern6[8] = {0x00, 0x1F, 0x00, 0x1F, 0x00, 0x1F, 0x00, 0x1F}; 	
	unsigned char data pattern7[8] = {0x00, 0x15, 0x00, 0x15, 0x00, 0x15, 0x00, 0x15}; //麻点
//int main() 
//{
//		
//while (1) 
//{
//		if (ReadKey1()) 
//	  {  
//		LCD_Clear();
//		mode++;
//		mode %= 10;			
//		switch (mode) 
//	  {
//        case 0:            
//			displayCustomCharacter1(pattern1); 
//            break;
//        case 1:
//            displayCustomCharacter1(pattern2);  
//            break;
//        case 2:
//            displayCustomCharacter1(pattern3); 
//            break;
//        case 3:
//           displayCustomCharacter1(pattern4);  
//            break;
//        case 4:
//            displayCustomCharacter1(pattern5);  
//            break;
//        case 5:
//            displayCustomCharacter1(pattern6);  
//            break;
//		case 6:
//			displayCustomCharacter1(pattern7);
//			break;
//		case 7:
//			write_CGROM(0x21); 
//		   break;
//		case 8:
//			write_CGROM(0xA1); 
//		   break;
//		case 9:
//			Write_DDRAM(0xFF); 
//		   break;
//    
//		}

//	LCD_Init(); 
//	LCD_Clear();
//	displayCustomCharacter1(pattern1); // 显示棋盘
//	Delay(200); 
//	displayCustomCharacter1(pattern2); 
//	Delay(200); 
//	displayCustomCharacter1(pattern3); // 显示竖线
//	Delay(200); 
//	displayCustomCharacter1(pattern4); 
//	Delay(200); 
//	displayCustomCharacter1(pattern5); // 显示横线
//	Delay(200); 
//	displayCustomCharacter1(pattern6); 
//	Delay(200); 
//	displayCustomCharacter1(pattern7); // 显示麻点
//	Delay(200); 
//	write_CGROM(0x21);//内置
//	Delay(200);
//	write_CGROM(0xA1);//内置
//	Delay(200);
//	Write_DDRAM(0xCE);//内置字符
//	Delay(200);
//	LCD_Clear();
//	LCD_ShowString(1,3,"LCM1602_10");
//	LCD_ShowString(0,2,"Made in China");
//	Delay(200);
//	}	
//		if (ReadKey1()) 
//	{  
//	LCD_Init(); 
//	LCD_Clear();
//	displayCustomCharacter1(pattern1); // 显示棋盘
//	Delay(500);
//	displayCustomCharacter1(pattern2); 
//	Delay(500);
//	displayCustomCharacter1(pattern3); // 显示竖线
//	Delay(500);
//	displayCustomCharacter1(pattern4); 
//	Delay(500);
//	displayCustomCharacter1(pattern5); // 显示横线
//	Delay(500);
//	displayCustomCharacter1(pattern6); 
//	Delay(500);
//	displayCustomCharacter1(pattern7); // 显示麻点
//	Delay(500);
//	write_CGROM(0x21);//内置
//	Delay(200);
//	write_CGROM(0xA1);//内置
//	Delay(200);
//	Write_DDRAM(0xCE);//内置字符
//	Delay(200);
//	LCD_Clear();
//	LCD_ShowString(1,3,"LCM1602_10");
//	LCD_ShowString(0,2,"Made in China");
//	Delay(200);
//	}	
//}

//	}

//void main(void)   
//{

//	LCD_Init(); 
//	LCD_Clear();

//    while (1)  
//    {
//      
//        if (ReadKey1()) 
//        {
//            HandleKey(); 
//        } 
//        else 
//        {
//          
//            DisplayPatterns(); 
//        }
//    }   
//}


//------------------------------------------
// 封装显示流程的函数
//------------------------------------------
void run_display(bit speedFlag)
{
    unsigned int delayTime = speedFlag ? 200 : 500;

    LCD_Init(); 
    LCD_Clear();

    displayCustomCharacter1(pattern1); Delay(delayTime);
    displayCustomCharacter1(pattern2); Delay(delayTime);
    displayCustomCharacter1(pattern3); Delay(delayTime);
    displayCustomCharacter1(pattern4); Delay(delayTime);
    displayCustomCharacter1(pattern5); Delay(delayTime);
    displayCustomCharacter1(pattern6); Delay(delayTime);
    displayCustomCharacter1(pattern7); Delay(delayTime);

    write_CGROM(0x21); Delay(200);
    write_CGROM(0xA1); Delay(200);
    Write_DDRAM(0xCE); Delay(200);
    LCD_Clear();
    LCD_ShowString(1,3,"LCM1602_10");
    LCD_ShowString(0,2,"Made in China");
    Delay(200);
}

bit speedFlag = 0;  // 0=慢速, 1=快速

void main(void)
{
    LCD_Init(); 
    LCD_Clear();

    // 上电时默认先执行一次慢速显示
    run_display(speedFlag);

    while(1)
    {
        if (ReadKey1())        // 检测按键按下
        {
            Delay(20);         // 消抖
            if (ReadKey1())    // 确认按下
            {
                speedFlag = !speedFlag;   // 切换速度档位
                while(ReadKey1());        // 等待松手
                run_display(speedFlag);   // 按一次键重新显示（速度改变）
            }
        }
    }
}

