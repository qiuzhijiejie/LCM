...
void LCD_Init() 
{
    LCD_WriteCommand(0x38); // 设置40×4显示,5X7点阵,8位数据接口 
...
void LCD_SetCursor(unsigned int Line, unsigned int Column) 
{
    ...
    {
        LCD_WriteCommand(0x80 | (Column + 40));  // 第三行
    } 
    else 
    {
...
/**
 * @brief  全屏显示AIP31066 内置字符
 * @param  Line 0-3  Column 0-39  输入16进制0X...
 * @retval 无
 */
void Write_DDRAM(unsigned char a)//用来寄存待显示的字符代码
{
    unsigned char j;
...
void DisplayPatterns() 
{
    ...
    switch (step) 
    {
        ...
        case 9:	LCD_Clear();LCD_ShowString(0,12,"YeHuiDisplay.com");LCD_ShowString(1,15,"LCM4004_02");Delay(80); break;
    }
    step++;
    if (step > 9) step = 0; // 循环
}