...
uchar ADJUST=50;	//对比度寄存器初始值	，vop调整设置  当前设置数值VDD=3.0V,VOP=13=8.0V
...
void InitST7567(void)
{
	...
	ST7567_WriteComd(0xc8);	//SHL select确定COM为正扫com63--->com0
 	Delay_Xms(2);
	ST7567_WriteComd(0x27);	//regulator resistor select V0调整电阻设定(1 + Rb/Ra)=5.0		0X26+0X41		vdd=5.0v,vop=10.0v						 0x26原来的数值
 	Delay_Xms(2);
...
{
	unsigned char k;
	char *words6[] = {X,ii,aa,mm,ee,nn,T,uu,oo,bb,ee,nn,T,ee,};
