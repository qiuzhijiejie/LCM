...
{
    unsigned int u32sel = 0, u32div = 0;

    if(MODULE_CLKDIV_Msk(u32ModuleIdx) != MODULE_NoMsk)
    {
        /* Get clock divider control register address */
        u32div = (unsigned int)&CLK->CLKDIV0 + ((MODULE_CLKDIV(u32ModuleIdx)) * 4);
        /* Apply new divider */
    ...
    if(MODULE_CLKSEL_Msk(u32ModuleIdx) != MODULE_NoMsk)
    {
        ...
        u32sel = (unsigned int)&CLK->CLKSEL0 + ((MODULE_CLKSEL(u32ModuleIdx)) * 4);
...
{
    *(volatile unsigned int *)((unsigned int)&CLK->AHBCLK + (MODULE_APBCLK(u32ModuleIdx) * 4))  |= 1 << MODULE_IP_EN_Pos(u32ModuleIdx);
}
...
{
    unsigned int u32PllSrcClk, u32NR, u32NF, u32NO, u32CLK_SRC;
    unsigned int u32Tmp, u32Tmp2, u32Tmp3, u32Min, u32MinNF, u32MinNR;
