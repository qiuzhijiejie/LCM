...
#line 1 "..\\lcd_driver\\VKL144A_IO_I2C_DIR.h"
...
#line 20 "..\\lcd_driver\\VKL144A_IO_I2C_DIR.h"
...
void VKL144A_Init(void);
void VKL144A_DisAll(unsigned char dat);
void VKL144A_DisDotOn(unsigned char seg,unsigned char com);
void VKL144A_DisDotOff(unsigned char seg,unsigned char com);
unsigned char VKL144A_Enter_PowerOff(void);
unsigned char VKL144A_Exit_PowerOff(void);

void VKL144A_Main(void);


 
#line 15 "..\\lcd_driver\\VKL144A_IO_I2C_DIR.c"
...
unsigned char VKL144A_dispram[18];
unsigned char VKL144A_readbuf[18];
unsigned char VKL144A_segi,VKL144A_comi;
 
unsigned char VKL144A_InitSequence(void);
...
unsigned char VKL144A_I2CSlaveAck( void )
{
...
void VKL144A_I2CWRCmd( unsigned char cmd )
{
	unsigned char i=8;
...
void VKL144A_I2CWRDat( unsigned char dat )
{
	unsigned char i=8;
	while (i--)
	{ 
...
unsigned char VKL144A_I2CRDDat( void )
{
...
unsigned char  WriteCmdVKL144A(unsigned char cmd)
{
	
	VKL144A_I2CStart(); 									
	
	VKL144A_I2CWRCmd(0x7C); 	
	if( 1 == VKL144A_I2CSlaveAck() )
	{
		VKL144A_I2CStop();													
		return 0;										
	}
	
	VKL144A_I2CWRCmd(cmd); 						
	if( 1 == VKL144A_I2CSlaveAck() )
	{
		VKL144A_I2CStop();													
		return 0;
...
unsigned char  WritenDataVKL144A(unsigned char Addr,unsigned char *Databuf,unsigned char Cnt)
{
	unsigned char n;
	...
	if(Addr>0x1f)													
		VKL144A_I2CWRCmd(0xEC); 	
	else                               
		VKL144A_I2CWRCmd(0xE8); 
	if( 1 == VKL144A_I2CSlaveAck() )
	{
		VKL144A_I2CStop();													
		return 0; 
	}
	VKL144A_I2CWRCmd(Addr&0x1f); 						
	if( 1 == VKL144A_I2CSlaveAck() )
	{
		VKL144A_I2CStop();													
		return 0;
	...
	for(n=0;n<Cnt;n++)
	{ 
		VKL144A_I2CWRDat(*Databuf++);
		if( VKL144A_I2CSlaveAck()==1 )
		{
			VKL144A_I2CStop();													
			return 0;
		}
...
unsigned char  ReadnDataVKL144A(unsigned char Addr,unsigned char *Databuf,unsigned char Cnt)
{
	unsigned char n;
...
void VKL144A_DisAll(unsigned char dat)
{
	...
	for(segi=0;segi<18;segi++)
	{
		VKL144A_dispram[segi]=dat;
	}
	WritenDataVKL144A(0,VKL144A_dispram,18);	
}
...
void VKL144A_DisDotOn(unsigned char seg,unsigned char com)
{
	...
	if((seg%2)==0)
		...
		addrbit=(1<<(4+com));
	tempdat=VKL144A_dispram[seg/2]|addrbit;
	VKL144A_dispram[seg/2]=tempdat;
	WritenDataVKL144A(addrbyte,&tempdat,1);		
}
...
{	
	...
	errorflag=VKL144A_InitSequence();
...
{			
  ...
  Delay_nuS(100);
	...
	VKL144A_I2CWRCmd(0xB8|0x00|0x04);  
...
void VKL144A_Lowlevel_Init(void)
{
...
{	
	...
	VKL144A_Lowlevel_Init();
	
	VKL144A_InitSequence();
}
...
{	
	...
	WriteCmdVKL144A(0xF3); 	
	
	
	
	WriteCmdVKL144A(0xFE);	
	Delay_nmS(3000);					
	
	WriteCmdVKL144A(0xFD);
	Delay_nmS(1000);					
	
	
	WriteCmdVKL144A(0xFC);
	WriteCmdVKL144A(0xF0);	
	...
	{		
		...
		VKL144A_DisAll(0x55);			
		ReadnDataVKL144A(0,VKL144A_readbuf,18);	
		Delay_nmS(1500);					
		VKL144A_DisAll(0xAA);			
		ReadnDataVKL144A(0,VKL144A_readbuf,18);	
		Delay_nmS(1500);					
		...
		VKL144A_DisAll(0x00);			
		VKL144A_Enter_PowerOff();	
		Delay_nmS(5000);					
		VKL144A_Exit_PowerOff();	
				
		
		VKL144A_DisAll(0x00);			
		Delay_nmS(500);						
		for(VKL144A_segi=0;VKL144A_segi<36;VKL144A_segi++)
		{
			for(VKL144A_comi=0;VKL144A_comi<4;VKL144A_comi++)
			{
				VKL144A_DisDotOn(VKL144A_segi,VKL144A_comi);		
				Delay_nmS(300);				
				VKL144A_DisDotOff(VKL144A_segi,VKL144A_comi);		
			}
		}
