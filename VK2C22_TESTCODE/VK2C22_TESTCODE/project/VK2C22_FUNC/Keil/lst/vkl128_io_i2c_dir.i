...
#line 1 "..\\lcd_driver\\VKL128_IO_I2C_DIR.h"
...
#line 20 "..\\lcd_driver\\VKL128_IO_I2C_DIR.h"
...
void VKL128_Init(void);
void VKL128_DisAll(unsigned char dat);
void VKL128_DisDotOn(unsigned char seg,unsigned char com);
void VKL128_DisDotOff(unsigned char seg,unsigned char com);
unsigned char VKL128_Enter_PowerOff(void);
unsigned char VKL128_Exit_PowerOff(void);

void VKL128_Main(void);


 
#line 15 "..\\lcd_driver\\VKL128_IO_I2C_DIR.c"
...
unsigned char VKL128_dispram[16];
unsigned char VKL128_readbuf[16];
unsigned char VKL128_segi,VKL128_comi;
 
unsigned char VKL128_InitSequence(void);
...
unsigned char VKL128_I2CSlaveAck( void )
{
...
void VKL128_I2CWRCmd( unsigned char cmd )
{
	unsigned char i=8;
...
void VKL128_I2CWRDat( unsigned char dat )
{
	unsigned char i=8;
	while (i--)
	{ 
...
unsigned char VKL128_I2CRDDat( void )
{
...
unsigned char  WriteCmdVKL128(unsigned char cmd)
{
	
	VKL128_I2CStart(); 									
	
	VKL128_I2CWRCmd(0x7C); 	
	if( 1 == VKL128_I2CSlaveAck() )
	{
		VKL128_I2CStop();													
		return 0;										
	}
...
unsigned char  WritenDataVKL128(unsigned char Addr,unsigned char *Databuf,unsigned char Cnt)
{
	unsigned char n;
	...
	VKL128_I2CWRCmd(0xE8); 
	if( 1 == VKL128_I2CSlaveAck() )
	{
		VKL128_I2CStop();													
		return 0; 
	}
	VKL128_I2CWRCmd(Addr&0x1f); 						
	if( 1 == VKL128_I2CSlaveAck() )
	{
		VKL128_I2CStop();													
		return 0;
	...
	for(n=0;n<Cnt;n++)
	{ 
		VKL128_I2CWRDat(*Databuf++);
		if( VKL128_I2CSlaveAck()==1 )
		{
			VKL128_I2CStop();													
			return 0;
		}
...
unsigned char  ReadnDataVKL128(unsigned char Addr,unsigned char *Databuf,unsigned char Cnt)
{
	unsigned char n;
...
void VKL128_DisAll(unsigned char dat)
{
	...
	{
		VKL128_dispram[segi]=dat;
	}
	WritenDataVKL128(0,VKL128_dispram,16);	
}
...
void VKL128_DisDotOn(unsigned char seg,unsigned char com)
{
	...
	if((seg%2)==0)
		...
		addrbit=(1<<(4+com));
	tempdat=VKL128_dispram[seg/2]|addrbit;
	VKL128_dispram[seg/2]=tempdat;
	WritenDataVKL128(addrbyte,&tempdat,1);		
}
...
{			
  ...
  Delay_nuS(100);
	...
	VKL128_I2CWRCmd(0xB8|0x00|0x04);  
...
void VKL128_Lowlevel_Init(void)
{
...
{	
	...
	{		
		...
		VKL128_DisAll(0x55);			
		ReadnDataVKL128(0,VKL128_readbuf,16);	
		Delay_nmS(1500);					
		VKL128_DisAll(0xAA);			
		ReadnDataVKL128(0,VKL128_readbuf,16);	
		Delay_nmS(1500);					
		...
		VKL128_DisAll(0x00);			
		VKL128_Enter_PowerOff();	
		Delay_nmS(5000);					
		VKL128_Exit_PowerOff();	
				
		
		VKL128_DisAll(0x00);			
		Delay_nmS(500);						
		for(VKL128_segi=0;VKL128_segi<32;VKL128_segi++)
		{
			for(VKL128_comi=0;VKL128_comi<4;VKL128_comi++)
			{
				VKL128_DisDotOn(VKL128_segi,VKL128_comi);		
				Delay_nmS(300);				
				VKL128_DisDotOff(VKL128_segi,VKL128_comi);		
			}
		}
